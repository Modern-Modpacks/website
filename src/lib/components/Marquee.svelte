<script lang="ts">
    ...
    export let shouldPlay : boolean = true // Weather the animation should play
...
<span
    ...
    on:mouseenter={() => {if ($reducedMotion || $mobile || animCount<2 || !stopDur) return; animPlaying = false; clearInterval(timeout ?? 0); if (shouldPlay) translate.set(get(translate) + (stopDist * (backwards ? -1 : 1)), {duration: stopDur, easing: sineOut})}} on:mouseleave={() => {if (!animPlaying) doAnim(false)}}
>
    <slot />
</span>