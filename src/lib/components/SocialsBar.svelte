...
<span class="{header ? "mt-2 " : "mobile:grid "}group flex gap-3 mobile:gap-4 w-fit mobile:grid-flow-col mobile:grid-rows-2">
    ...
    {@const social = socials[k]}
        <BreathingIcon duration={2000} minScale={85} maxScale={100} delay={1500 + (400 * +i)} play={header} class="duration-200 desktop:group-hover:!scale-100">
            <a href="{social.url}" target="_blank" rel="noopener noreferrer" title="{k}" class="{header ? "opacity-0 translate-y-10 duration-[.5s]" : `rounded-full ${$lightMode ? "bg-text-light" : "bg-text-dark"} block p-1.5 motion-safe:desktop:hover:scale-125 duration-300`}" style="{header ? `transition-delay: ${1 + (.05 * +i * +header)}s;` : ""}">
