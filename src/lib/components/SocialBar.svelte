...
<span class="{header ? "mt-2 " : ""}group flex gap-3 w-fit">
    ...
    {@const social = socials[k]}
        <BreathingIcon duration={2000} minScale={85} maxScale={100} delay={1500 + (400 * +i)} play={header} class="duration-200 group-hover:!scale-100">
            <a href="{social.url}" title="{social.title}" class="{header ? " opacity-0 translate-y-10 duration-[.5s]" : ""}" style="transition-delay: {1 + (.05 * +i * +header)}s;">
                <img src="{icons[k]}" id="animinheader" alt="website logo" class="h-10 brightness-0 invert motion-safe:hover:invert-[55%] duration-200">
            </a>
        </BreathingIcon>
    {/each}
</span>