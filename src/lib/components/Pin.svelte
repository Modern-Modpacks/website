...
<span class="absolute top-0 left-0 flex flex-col items-center cursor-pointer group" style="translate: {(pin.coords.x / ($mobile ? 2 : 1)) - (12 * +$reducedMotion)}px {pin.coords.y / ($mobile ? 1.4 : 1)}px;" title="{$_("languages."+pin.lang)}" on:click={() => {$activatedPin = (active ? null : pin)}}>
